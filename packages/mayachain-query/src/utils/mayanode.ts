...
export class Mayanode {
  ...
  private mayanamesApi: MayanamesApi[]
  ...
  constructor(network: Network = Network.Mainnet, config?: MayanodeConfig) {
    ...
    this.saversApi = this.config.mayanodeBaseUrls.map((url) => new SaversApi(new Configuration({ basePath: url })))
    ...
    this.mayanamesApi = this.config.mayanodeBaseUrls.map(
      (url) => new MayanamesApi(new Configuration({ basePath: url })),
    )
  }
  ...
  async getMayanameDetails(mayaname: string, height?: number): Promise<Mayaname> {
    for (const api of this.mayanamesApi) {
      try {
        const resp = (await api.mayaname(mayaname, height)).data
        return resp
      } catch (e) {}
    }
    throw new Error(`MAYANode is not responding`)
