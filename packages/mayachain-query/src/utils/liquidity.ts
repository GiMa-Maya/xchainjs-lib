...
export const getLiquidityUnits = (liquidity: LiquidityToAdd, pool: LiquidityPool): BigNumber => {
  const baseAmount10decimals = getBaseAmountWithDiffDecimals(liquidity.asset, 10)
  const P = new BigNumber(pool.mayanodeDetails.LP_units)
...
/**
 *
 * @param unitData - units for both asset and cacao
 * @param pool - pool that the asset is bound to
 * @returns - pool share of both asset and cacao in percentage
 */
export const getPoolShare = (unitData: UnitData, pool: LiquidityPool): PoolShareDetail => {
  // formula: (cacao * part) / total; (asset * part) / total
  ...
  const cacao = R.times(units).div(total)
  const poolShareDetail = {
    assetShare: new CryptoAmount(baseAmount(asset), pool.asset),
    cacaoShare: new CryptoAmount(baseAmount(cacao), AssetMAYA),
  }
...
/**
 *
 * @param poolShare - the share of asset and cacao added to the pool
...
export const getLiquidityProtectionData = (
  ...
  const R0 = depositValue.cacao.amount() // cacao deposit value
  const A0 = depositValue.asset.amount() // asset deposit value
  const R1 = poolShare.cacaoShare.baseAmount.amount() // cacao amount to redeem
  ...
  const ILProtection: ILProtectionData = {
    ILProtection: new CryptoAmount(baseAmount(maxILP), AssetMAYA),
